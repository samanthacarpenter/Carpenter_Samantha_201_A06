...
for(i in 1:length(player_info)){
  player_ppg <- player_info[[i]]$points/player_info[[i]]$games_played
  names_of_players <- append(names_of_players, names(player_info)[[i]])
  points_per_game <- append(points_per_game, player_ppg)
}

names(points_per_game) <- names_of_players
points_per_game
```
...
```{r}
rounded_ppg <- sapply(points_per_game, function(x){round(x,2)})
rounded_ppg
```
...
```{r}
apply(mat_rand, MARGIN = 1:2, function(x){signif(x, 1)})
```
...
```{r}
apply(max_temps[c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')], 2, mean)
```
...
```{r}
med_temps <- apply(CO_max_temps[c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')], 1, median)

max_index <- which.max(med_temps)
min_index <- which.min(med_temps)

paste0('The maximum median annual temperature of ', med_temps[max_index], ' occurred in ', CO_max_temps$year[max_index], ' and the minimum median annual temperature of ', med_temps[min_index], ' occurred in ', CO_max_temps$year[min_index], '.')
```
...
```{r}
digit_sum <- function(user_num){
  running_sum <- 0
  if(user_num == 0){
    return(0)
  } else{
    running_sum <- running_sum + user_num %% 10
    return(running_sum + digit_sum(user_num %/% 10))
  }
}
