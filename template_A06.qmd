...
[https://github.com/samanthacarpenter/Carpenter_Samantha_201_A06](https://github.com/samanthacarpenter/Carpenter_Samantha_201_A06)
...
![](Screenshot 2024-05-20 at 11.42.51 AM Small.png)
...
```{r}
player_info[[2]][[4]] # index 4th element of 2nd element of list
player_info$LJ$points # using names
```
...
```{r}
height <- c(78, 81, 78, 86, 85) # put heights into vector

for(i in 1:length(player_info)){ # iterates through each player in list
  player_info[[i]]$height <- height[[i]] # assigns respective height to each player, names element
}
...
```{r}
points_per_game <- list() # initialize empty list of points per game
names_of_players <- c() # initialize empty vector of player names

for(i in 1:length(player_info)){ # iterates through each player
  player_ppg <- player_info[[i]]$points/player_info[[i]]$games_played # calculates points per game for specific player
  names_of_players <- append(names_of_players, names(player_info)[[i]]) # name of specific player is added to vector of player names
  points_per_game <- append(points_per_game, player_ppg) # points per game value is added to list
}

names(points_per_game) <- names_of_players # each element is named with respective player
points_per_game
```
...
```{r}
rounded_ppg <- sapply(points_per_game, function(x){round(x,2)}) # sapply to return vector, uses customized round() function to round to 2 decimal places
rounded_ppg
```
...
```{r}
apply(mat_rand, MARGIN = 1:2, function(x){signif(x, 1)}) # function applies to each value, uses customized signif() function to round to 1 significant figure
```
...
```{r}
apply(max_temps[c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')], 2, mean) # only applies to columns for each month, calculates means
```
...
```{r}
CO_max_temps <- max_temps[max_temps$state == 'CO',] # only keeps values where state is CO
OH_max_temps <- max_temps[max_temps$state == 'OH',] # only keeps values where state is OH
VA_max_temps <- max_temps[max_temps$state == 'VA',] # only keeps values where state is VA
```
...
```{r}
med_temps <- apply(CO_max_temps[c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')], 1, median) # calculates median temps for each year using only data from columns for each month

max_index <- which.max(med_temps) # finds index of maximum median temperature
min_index <- which.min(med_temps) # finds index of minimum median temperature

paste0('The maximum median annual temperature of ', med_temps[max_index], ' occurred in ', CO_max_temps$year[max_index], ' and the minimum median annual temperature of ', med_temps[min_index], ' occurred in ', CO_max_temps$year[min_index], '.') # prints statement using index values of max and min to find the correct temperatures and their respective years
```
...
```{r}
digit_sum <- function(user_num){ # takes number as input
  if(user_num == 0){ # base case returns 0 when out of digits
    return(0)
  } else{
    return(user_num %% 10 + digit_sum(user_num %/% 10)) # returns sum of last digit of number and result of truncated new number when run through function again
  }
}
